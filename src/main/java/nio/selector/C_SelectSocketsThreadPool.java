 ...
 */
public class C_SelectSocketsThreadPool extends B_SelectSockets
{
	...
	public static void main (String [] argv)
		throws Exception
	{
		new C_SelectSocketsThreadPool().go (argv);
	}
	...
	private class WorkerThread extends Thread
	{
		...
		/**
		 * 这个键的兴趣集是
		 * 更新删除op_read。这将导致选择器,忽略对这个通道的读准备
		 * 这将防止选择器重复地调用readDataFromSocket( )（因为通道仍然会准备好读取数据，直到工作线程从它那里读取数据）。
		 * @param key
		 */
		 ...
		 * 在调用之前修改了key以关闭选择感兴趣的操作OP_READ,
		 * 当此方法完成时,在选择器上重新启用OP_READ和调用wakeup,这样选择器能重新匹配这个通道
		 *
		 * 当工作线程结束为通道提供的服务时，它将再次更新键的ready集合，来将interest重新放到读取就绪集合中。它也会在选择器上显式地调用wakeup( )。
		 * 如果主线程在select( )中被阻塞，这将使它继续执行。这个选择循环会再次执行一个轮回（可能什么也没做）并带着被更新的键重新进入select( )。
		 */
