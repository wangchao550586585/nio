...
/**
 * 使用DatagramChannel发送请求到多个地址上的时间服务器。DatagramChannel接着会等待回复（reply）的到达。
 ...
 */
public class K_TimeClient {
    private static final int DEFAULT_TIME_PORT = 37;
    private static final long DIFF_1900 = 2208988800L;
    //表示自1900年1月1日起的秒数。java时代开始于1970年1月1日（同Unix），调整酌情添加或减去2208988800
    ...
    public K_TimeClient(String[] argv) throws Exception {
        if (argv.length == 0) {
    ...
    public static void main(String[] argv)
            throws Exception {
        argv = new String[]{"-p", "37","127.0.0.1"};
        K_TimeClient client = new K_TimeClient(argv);
